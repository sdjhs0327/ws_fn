...
def imputation(df, basis_name, target_name, log_diff=True):
    ...
    if log_diff:
        ...
        ## 원천 스케일로 되돌리기
        last_idx = df[target_name].dropna().index[-1]
        last_ratio = df[target_name][last_idx]/dataset[target_name][last_idx]
        dataset[target_name] = dataset[target_name]*last_ratio
    else:
